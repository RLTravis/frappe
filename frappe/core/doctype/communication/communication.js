frappe.ui.form.on("Communication", {
	...
	reply: function (frm) {
		...
		let all_recipients = frm.doc.sender.concat(frm.doc.recipients)
		$.extend(args, {
			subject: __('Re: {0} (#{1})', [frm.doc.subject, frm.doc.reference_name]),
	...
	reply_all: function (frm) {
		...
		let all_recipients = frm.doc.sender.concat(frm.doc.recipients)
		$.extend(args, {
			subject: __('Res: {0} (#{1})', [frm.doc.subject, frm.doc.reference_name]),
			recipients: all_recipients,
			cc: frm.doc.cc
		})
		new frappe.views.CommunicationComposer(args);
	...
	forward_mail: function (frm) {
		...
		$.extend(args, {
			forward: true,
			subject: __('Fw: {0} (#{1})', [frm.doc.subject, frm.doc.reference_name]),
		})
